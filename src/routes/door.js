      ...
      Orders.updateOne(
        { _id: new ObjectId(car_ctrl.target.order._id) },
        {
          $set: {
            ...
            time_start: car_ctrl.target.order.status == "arrive_at_get" ?
              moment().format("YYYY-MM-DD HH:mm:ss") : car_ctrl.target.order.time_start,
            time_end: car_ctrl.target.order.status == "arrive_at_send" ?
              moment().format("YYYY-MM-DD HH:mm:ss") : car_ctrl.target.order.time_end
          }
        }
      );
    }
